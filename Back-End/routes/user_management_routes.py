...
# get all users list
@user_management_bp.route('/userlist', methods=['GET'])
@jwt_required()
def get_all_users():
...
def delete_user(user_id):
    ...
    # avoid admin deleting their own account
...
def update_user(user_id):
    ...
    # updating email, check if it already exists
    if 'email' in data and data['email'] != user.email:
        existing_email = UserInformation.objects(email=data['email']).first()
        if existing_email:
            return jsonify({"error": "Email already exists"}), 400

    # not allow direct password updates, use a separate password change endpoint
    if 'password' in data:
        del data['password']
...
# remove house from wishlist, not sure if needed
